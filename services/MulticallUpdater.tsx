...
async function fetchChunk(
    ...
    try {
        const { blockNumber, returnData } = await multicall2Contract.callStatic.aggregate(
            chunk.map((obj) => ({
                target: obj.address,
 ...
 */
export function activeListeningKeys(allListeners: CallListeners, chainId?: number): { [callKey: string]: number } {
...
export default function Updater(): null {
    const { multicallModel } = useStoreState(state => state)
    const actions = useStoreActions(actions => actions)
    // wait for listeners to settle before triggering updates
    const debouncedListeners = useDebounce(multicallModel.callResults, 100)
    ...
    const unserializedOutdatedCallKeys = useMemo(() => {
        return outdatedListeningKeys(multicallModel.callResults, listeningKeys, chainId, latestBlockNumber)
    }, [chainId, multicallModel.callResults, listeningKeys, latestBlockNumber])
    ...
    useEffect(() => {
        ...
        actions.multicallModel.fetchingMulticallResults({
            calls,
            chainId,
                ...
                promise
                    .then(({ results: returnData, blockNumber: fetchBlockNumber }) => {
                        ...
                        if (results && Object.keys(results).length > 0) {
                            actions.multicallModel.updateMulticallResults({
                                chainId,
                                results,
                        ...
                        if (erroredCalls.length > 0) {
                            ...
                            actions.multicallModel.errorFetchingMulticallResults({
                                calls: erroredCalls,
                                chainId,
    ...
    }, [chainId, multicall2Contract, serializedOutdatedCallKeys, latestBlockNumber])
