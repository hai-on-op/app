...
export const getRatePercentage = (value: string, digits = 4, returnRate = false) => {
    ...
    const ratePercentage = rate < 1 ? (numeral(1).subtract(rate).value() || 0) * -1 : numeral(rate).subtract(1).value() || 0
...
export const getCollateralRatio = (
    ...
    return formatNumber(value.value()?.toString() || '0', 2, true)
}
...
export const getLiquidationPrice = (
    ...
    return formatNumber(numerator.value()?.toString() || '0')
}
...
export const getInterestOwed = (debt: string, accumulatedRate: string) => {
    ...
    return formatNumber(numeral(debt).multiply(restAcc).value()?.toString() || '0', 4, true)
}
...
export const returnFiatValue = (value: string, price: number) => {
    ...
    return formatNumber(numeral(value).multiply(price).value()?.toString() || '0', 2)
}
