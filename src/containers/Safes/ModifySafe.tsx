...
const ModifySafe = ({ isDeposit, isOwner }: Props) => {
    ...
    const collateralUnitPriceUSD = formatNumber(
        liquidationData!.collateralLiquidationData[singleSafe!.collateralName].currentPrice.value
    )
    ...
    const debtFloorBN = BigNumber.from(
        toFixedString(
            liquidationData!.collateralLiquidationData[singleSafe!.collateralName].debtFloor,
            'WAD'
        )
    )

    const safetyRatio = liquidationData!.collateralLiquidationData[singleSafe!.collateralName].safetyCRatio
    const safetyRatioBN = BigNumber.from(Number(safetyRatio) * 100)

    const selectedTokenDecimals = singleSafe
        ? tokenBalances[singleSafe.collateralName]?.decimals || '18'
        : '18'
    ...
    const [collateralUnlockState, collateralApproveUnlock] = useTokenApproval(
        ...
        singleSafe
            ? tokensData[singleSafe.collateralName!]?.address
            : undefined,
        proxyAddress,
    ...
    const currentRedemptionPrice = singleSafe!.currentRedemptionPrice
    ...
    const onMaxLeftInput = () => {
        if (isDeposit) return onLeftInput(depositTokenBalance.toString())
        
        const currentColRatio = singleSafe?.collateralRatio
    ...
    const onMaxRightInput = () => {
        if (isDeposit) return onRightInput(availableHai)
    ...
    const handleSubmit = () => {
        safeActions.setSafeData({
            ...
            collateral: singleSafe?.collateralName || 'WETH',
        })
    ...
    return (
        <Container>
            ...
            <Inner>
                <InputBlock>
                    <SideLabel>
                        {isDeposit ? 'Deposit': 'Withdraw'} {singleSafe.collateralName}
                    </SideLabel>
                    ...
                    <TokenInput
                        ...
                        token={{
                            ...
                            icon: (TOKEN_LOGOS as any)[singleSafe.collateralName],
                        }}
                        label={isDeposit
                            ? `Balance: ${formatNumber(leftInputBalance.toString(), 3)} ${singleSafe.collateralName}`
                            : `Available: ${formatNumber(leftInputBalance.toString(), 3)} ${singleSafe.collateralName}`
                        }
                    ...
                    : !isDeposit
                        ? (unlockState === ApprovalState.PENDING || unlockState === ApprovalState.NOT_APPROVED)
                            ...
                            : (
                                <Button
                                    onClick={handleSubmit}
                                    disabled={!isValid || !isSuccessfulTx}>
                                    {!isSuccessfulTx ? 'Pending Transaction...' : 'Review Transaction'}
                                </Button>
                            )
                        : (collateralUnlockState === ApprovalState.PENDING || collateralUnlockState === ApprovalState.NOT_APPROVED)
                            ? (
                                <Button
