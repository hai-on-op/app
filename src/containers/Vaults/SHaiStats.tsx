...
export function SHaiStats() {
    ...
    const { combined, isLoading: statsLoading } = useMinterStats('sHai', aeroPriceUsd)
    ...
    const debtCapacityFormatted = useMemo(() => {
        const pct = graphSummary?.collateralStats?.['SHAI']?.debt?.ceilingPercent
    ...
    const sHaiBaseApr = ((strategyData?.haiAero?.boostApr as any)?.baseAPR || 0) / 100

    const aprFormatted = useMemo(() => {
        return formatNumberWithStyle(sHaiBaseApr * 100, { style: 'percent', suffixed: true, maxDecimals: 2 })
    }, [sHaiBaseApr])
