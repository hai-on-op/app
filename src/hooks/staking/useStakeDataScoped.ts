...
export function useStakeDataScoped(namespace: string = 'kite') {
    ...
    const data = useMemo(() => {
        return {
            ...
            pendingWithdrawal: account.data?.pendingWithdrawal || null,
            cooldownPeriod: account.data?.cooldown || 0,
            totalStaked: stats.data?.totalStaked || '0',
        }
    }, [account.data?.stakedBalance, account.data?.pendingWithdrawal, account.data?.cooldown, stats.data?.totalStaked])
