...
const USER_SAFES_QUERY = gql`
  query GetSafesByCollateralTypeAndOwner(
    $collateralTypeId: ID!, 
    $ownerId: ID, 
    $limit: Int = 1000
  ) {
    safes(
      where: {
        collateralType_: {id: $collateralTypeId},
        owner_: {id: $ownerId}
      },
      orderBy: collateral,
      orderDirection: desc,
 ...
 const { address } = useAccount()
  ...
  const formattedData = useMemo(() => {
    ...
    if (userSafesData?.safes && userSafesData.safes.length > 0) {
      ...
      const totalUserCollateral = userSafesData.safes.reduce(
    ...
    return {
      totalHaiVELODeposited: formattedTotal.formatted || '0',
      userHaiVELODeposited: userDeposited
    }
  }, [data, userSafesData])
