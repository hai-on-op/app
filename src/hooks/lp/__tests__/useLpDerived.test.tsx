...
describe('LP derived hooks', () => {
    ...
    function ValueComp({ address }: { address: `0x${string}` }) {
        ...
        const value = React.useMemo(() => {
            ...
            return lpService.calculateUserPositionsValue(
    ...
    it('useLpCurrentPositions composes from pool + user positions', async () => {
        vi.spyOn(poolHook, 'useLpPool').mockReturnValue({
            data: {
                id: 'pool',
                liquidity: '0',
        ...
        vi.spyOn(userPositionsHook, 'useLpUserPositions').mockReturnValue({
            data: [
                {
                    id: '1',
                    liquidity: '1',
    ...
    it('useLpUserPositionValue computes USD value from positions and prices', async () => {
        ...
        vi.spyOn(currentPositionsHook, 'useLpCurrentPositions').mockReturnValue({
            data: [
                {
                    id: '1',
                    liquidity: '1',
        ...
        vi.spyOn(analyticsProvider, 'useAnalytics').mockReturnValue({
            haiMarketPrice: { raw: '1' },
            data: { tokenAnalyticsData: [{ symbol: 'WETH', currentPrice: (2000n * 10n ** 18n).toString() }] },
        } as any)
        vi.spyOn(veloProvider, 'useVelodromePrices').mockReturnValue({
            prices: { VELO: { raw: '1' } },
            loading: false,
        } as any)
