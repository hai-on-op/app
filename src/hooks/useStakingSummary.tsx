...
export function useStakingSummary(): StakingSummaryData {
  ...
  const effectiveStakedBalance = useMemo(() => {
    ...
    const pendingAmount = stakingData.pendingWithdrawal ? 
      Number(String(stakingData.pendingWithdrawal.amount || '0')) : 0
  ...
  const stakingSummary = useMemo(() => {
    ...
    const numericHaiVeloPositionValue = typeof haiVeloPositionValue === 'string' ? 
      Number(haiVeloPositionValue) : haiVeloPositionValue || 0

    const numericUserLPPositionValue = typeof userLPPositionValue === 'string' ? 
      Number(userLPPositionValue) : userLPPositionValue || 0
    ...
    return {
      ...
      boost: {
        netBoostValue,
        netBoostFormatted: formatNumberWithStyle(netBoostValue, {
          minDecimals: 0,
          maxDecimals: 2,
        }) + 'x',
