...
import { calculateHaiVeloBoost, calculateVaultBoost } from '~/services/boostService'
...
export function useEarnStrategiesOld() {
    const {
    ...
    const calculateVaultBoostAPR = useCallback(
        (cType) => {
            // Guard against running with incomplete data
            if (!totalStaked || !Object.keys(usersStakingData).length) {
                return {
                    userVaultBoostMap: {},
                    cType: cType.id,
            ...
            if (!ctypeMinterData) return null
            console.log('minterVaultsData', minterVaultsData)
            console.log('ctypeMinterData', ctypeMinterData)

            const userVaultBoostMap: Record<string, number> = Object.entries(
                ctypeMinterData?.userDebtMapping || {}
            ).reduce((acc, [address, value]) => {
                const lowercasedAddress = address.toLowerCase()
                if (!usersStakingData[lowercasedAddress]) return { ...acc, [lowercasedAddress]: 1 }

                return {
                    ...acc,
                    [lowercasedAddress]: calculateVaultBoost({
                        userStakingAmount: Number(usersStakingData[lowercasedAddress]?.stakedBalance),
                        totalStakingAmount: Number(totalStaked),
                        userVaultMinted: Number(value),
                        totalVaultMinted: Number(ctypeMinterData?.totalMinted),
                    }).vaultBoost,
                }
            }, {})
            ...
            const dailyKiteReward = REWARDS.vaults[cType.id as keyof typeof REWARDS.vaults]?.KITE || 0

            const dailyKiteRewardUsd = dailyKiteReward * (prices?.KITE || 0)
            ...
            // const userPosition = list.filter(({ collateralName }) => collateralName === cType.id)

            // const userPositionList = list[Symbo
            return {
                userVaultBoostMap,
                cType: cType.id,
        ...
        [address, minterVaultsData, usersStakingData, totalStaked, userPositionValuesMap]
    )
        ...
        return (
            data.collateralTypes
                ...
                .map((cType) => {
                    ...
                    // const apr = calculateAPR(parseFloat(cType.debtAmount) * prices.HAI, prices, rewards)
                    const vbr = calculateVaultBoostAPR(cType)
                    console.log('vbr', vbr)

                    return {
                        pair: [symbol || 'HAI'],
                        rewards: Object.entries(rewards).map(([token, emission]) => ({ token, emission })),
                        tvl: (parseFloat(cType.debtAmount) * prices.HAI) as any,
                        strategyType: 'borrow',
                        boostAPR: vbr as any,
                        apr: '0' as any,
                        userPosition: list
    ...
    }, [data?.collateralTypes, prices, list, tokenAssets, calculateVaultBoostAPR])
    ...
    const uniStrategies = useMemo(() => {
        ...
        for (const pool of uniData.liquidityPools) {
            const rewards = REWARDS.uniswap[pool.id.toLowerCase() as keyof typeof REWARDS.uniswap]
    ...
    const veloStrategies = useMemo(() => {
        ...
        const temp: Strategy[] = []
        // Filter out SAIL
        for (const pool of veloData.filter(
            (p) =>
                p.address === '0xf2d3941b6E1cbD3616061E556Eb06986147715d1' ||
                p.address === '0x056B153132F105356d95CcF34a0065A28617DaC4'
        )) {
    ...
    const calculateHaiVeloBoostAPR = useCallback(() => {
        ...
        console.log('<<<<<<< userHaiVeloBoostMap >>>>>>', userHaiVeloBoostMap)
    ...
    const specialStrategies = useMemo(() => {
        ...
        console.log('--------------------------------')
        console.log('haiVeloBoostData', haiVeloBoostData)
        console.log('--------------------------------')
    ...
    // console.log('specialStrategies', specialStrategies)
    ...
    }, [isLoading, vaultStrategies])

    // console.log('--------------------------------')
    // console.log('vaultStrategies', vaultStrategies)
    // console.log('--------------------------------')
    ...
    const averageAPR = useMemo(() => {
        ...
        // console.log('effectiveStrategiesAPR', effectiveStrategiesAPR)
...
const calculateAPY = (
    tvl: number,
    prices: { KITE: number; OP: number; [key: string]: number } | null | undefined,
    rewards: { KITE: number; OP: number } = REWARDS.default
) => {
    if (!tvl || !prices) return 0
    if (!prices.KITE || !prices.OP) return 0

    const annualRewardValue = (rewards.KITE * prices.KITE + rewards.OP * prices.OP) * 365
    const nominal = annualRewardValue / tvl

    if (nominal === Infinity) return 0

    // APY with monthly compounding
