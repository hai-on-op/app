...
const multicallModel: MulticallModel = {
    callResults: {},
    addMulticallListeners: action((state, { calls, chainId, options: { blocksPerFetch = 1 } = {} }) => {
    ...
    removeMulticallListeners: action((state, { chainId, calls, options: { blocksPerFetch = 1 } = {} }) => {
        const listeners = state.callListeners ? state.callListeners : (state.callListeners = {})
    ...
    fetchingMulticallResults: action((state, { chainId, fetchingBlockNumber, calls }) => {
    ...
    errorFetchingMulticallResults: action((state, { fetchingBlockNumber, chainId, calls }) => {
        state.callResults[chainId] = state.callResults[chainId] ?? {}
        calls.forEach((call) => {
            const callKey = toCallKey(call)
            const current = state.callResults[chainId][callKey]
    ...
    updateMulticallResults: action((state, { chainId, results, blockNumber }) => {
        state.callResults[chainId] = state.callResults[chainId] ?? {}
